...
- Infisical admin credentials → log into Infisical UI, generate a new client secret for the `claude-code` machine identity, recreate `/home/matthieu/.config/infisical-claude` (chmod 600) on the server — required for Claude Code tooling and `manual_xyops_update.sh`
